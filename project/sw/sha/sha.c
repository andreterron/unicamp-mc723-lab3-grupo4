...
void sha_stream(SHA_INFO *sha_info,BYTE *data,int size)
{    
    sha_init(sha_info);
    sha_update(sha_info, data, size);
    sha_final(sha_info);
}
...
/* print a SHA digest to file */
void sha_print_to_file(SHA_INFO *sha_info,FILE *fileP)
{
    fprintf(fileP,"%08lx %08lx %08lx %08lx %08lx\n",
    sha_info->digest[0], sha_info->digest[1], sha_info->digest[2],
    sha_info->digest[3], sha_info->digest[4]);
}
